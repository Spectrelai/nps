...
env GOOS=linux GOARCH=mips GOMIPS=softfloat CGO_ENABLED=1 CC=mips-linux-gnu-gcc go build -buildmode=c-archive -o libnpc_mips_sdk.a cmd/npc/sdk.go
env GOOS=linux GOARCH=mips GOMIPS=softfloat CGO_ENABLED=1 CC=mips-linux-gnu-gcc go build -buildmode=c-shared -o libnpc_mips_sdk.so cmd/npc/sdk.go
tar -czvf npc_linux_sdk.tar.gz libnpc_arm_sdk.a libnpc_arm_sdk.so libnpc_arm_sdk.h libnpc_mips_sdk.a libnpc_mips_sdk.so libnpc_mips_sdk.h
