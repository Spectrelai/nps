...
env GOOS=linux GOARCH=mipsle CGO_ENABLED=1 CC=mipsel-linux-gnu-gcc go build -buildmode=c-archive -o libnpc_mipsel_sdk.a cmd/npc/sdk.go
env GOOS=linux GOARCH=mipsle CGO_ENABLED=1 CC=mipsel-linux-gnu-gcc go build -buildmode=c-shared -o libnpc_mipsel_sdk.so cmd/npc/sdk.go
tar -czvf npc_linux_sdk.tar.gz libnpc_arm_sdk.a libnpc_arm_sdk.so libnpc_mipsel_sdk.a libnpc_mipsel_sdk.so libnpc_arm_sdk.h
